...
class CreateManyDirective extends BaseDirective {
  visitFieldDefinition(field) {
    ...
    field.resolve = async function (item, { input }) {
      let res = await Model.createMany(input);
  ...
  _checkArguments(field) {
    ...
    const kind = this._getTypeIgnoreNonNull(
      field.args.find((e) => e.name === "input").astNode.type
    );
    if (kind !== "ListType")
      throw new Error(
        "The @createMany directive only accepts an array of models"
      );

    const returnType = this._getTypeIgnoreNonNull(field.astNode.type);
    if (returnType !== "ListType")
      throw new Error(
        "The @createMany directive only returns an array of models"
      );
  }
}
